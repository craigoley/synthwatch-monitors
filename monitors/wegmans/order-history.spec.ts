...
test("Order History Terraform flow", async ({ page }) => {
  ...
  await step("Open My Orders & Receipts", async () => {
    const greeting = page
      .getByRole("button", { name: /hello|account|my wegmans|rewards/i })
      .or(page.getByRole("link", { name: /hello|account|my wegmans|rewards/i }))
      .filter({ visible: true })
    ...
    const orders = page
      .getByRole("link", { name: /my orders/i })
      .or(page.getByRole("button", { name: /my orders/i }))
      .filter({ visible: true })
      .first();
    await expect(orders).toBeVisible({ timeout: 30_000 });
    await orders.click();
    await page.waitForLoadState("domcontentloaded");
  });

  await step("Verify an order is listed", async () => {
    // TF validates an order detail link (href^="/order/") carrying "View" text
    // renders -- this only appears once the order-history list finishes loading.
