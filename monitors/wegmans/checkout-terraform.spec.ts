...
test('Checkout Terraform flow', async ({ page }) => {
  ...
  await step('Set pickup fulfillment context', async () => {
  ...
  await step('Open cart and proceed to checkout', async () => {
  ...
  await step('Choose a time slot gate', async () => {
    ...
    await expect(page.getByText(/payment/i).first()).toBeVisible({ timeout: 30_000 });
  });
});
