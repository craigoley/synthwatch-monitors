...
test("Digital Coupons Terraform flow", async ({ page }) => {
  ...
  await step("Open Digital Coupons", async () => {
    const coupons = page
      .getByRole("link", { name: /coupon/i })
      .or(page.getByRole("button", { name: /coupon/i }))
      .filter({ visible: true })
      .first();
    ...
    // TF validates the presence of the "Clip" affordance on a coupon tile --
    // this only renders once the coupons list has actually loaded.
