...
/**
 * Monitor: amore-reservations
 *
 * Journey: wegmansamore.com/reservations -> assert the OpenTable booking widget LOADS.
 *
 * Amore (sit-down Italian, Rochester East Ave) takes reservations via an EMBEDDED
 * OpenTable widget (rid=107458). The business-critical capability is "a guest can
 * reach the booking widget" -- this monitor NEVER books and NEVER submits anything.
 *
 * ★ GROUND TRUTH (recon 2026-06-30): loading /reservations/ fires the OpenTable widget
 * loader REQUEST: GET https://www.opentable.com/widget/reservation/loader?rid=107458 (the
 * embed bootstraps the iframe from this). Two flaky signals we deliberately AVOID:
 *   - the OpenTable <iframe> does NOT reliably inject headless (iframeAppeared=false), and
 ...
 * up the OpenTable booking embed (rid=107458). We assert that + the "RESERVE YOUR TABLE"
...
test('Amore: reservations OpenTable widget loads', async ({ page }) => {
  ...
  await step('open the reservations page', async () => {
    await page.goto('https://wegmansamore.com/reservations/', { waitUntil: 'domcontentloaded' });
  ...
  await step('assert the reservations affordance rendered', async () => {
    ...
    await expect(
      ...
      'Amore reservations: the "RESERVE YOUR TABLE" heading did not render.',
